...
如果一个javabean对象可能被多个线程访问,虽然在程序代码顺序上是先set赋值然后再传递给其它线程,但是可能会有重排序  
会导致一个没有完全set设置属性的对象提前发布到另外一个线程中,这样就会有线程安全问题,解决办法构建HB原则,保证安全  
发布,保证在多线程中一个线程能够看见一个完整的状态正确的对象  
...
在多线程中使用,会出现对象状态不一致问题,如果一定要用必须要进行同步  
