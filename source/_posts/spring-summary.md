...
4.如果数据库连接池连接被设置为不自动提交,当前事务方法调用的类也没有使用@Transanction注解,那么不会创建代理,也不会执行TransanctionInterceptor,  
那么就没有事务管理,执行sql不会被提交,而且也是新获取数据库连接,也不会被关闭吧  

5.在没有数据库操作的类上加了@Transaction注解有什么影响  
创建了没必要的代理,然后拦截器会创建事务信息txObject和一些事务同步状态的设置,这就是全部开销,如果不加注解容易产生bug      
